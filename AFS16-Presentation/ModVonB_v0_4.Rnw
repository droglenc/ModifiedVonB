...
\bigskip
\begin{itemize}
  \item The \textit{inverse VBGF} is found by algebraically solving for $t$.
  \pause
  \smallskip
...
\[\Scale[1.5]{ t = \frac{log_{e}\left(1-\frac{L_{t}}{L_{\infty}}\right)}{-K} + t_{0} }\]
  ...
  \item ... and that the critical length of interest is 300.
  \pause
  \bigskip
...
\[\Scale[1.5]{ L_{t} = \bm{L_{r}} + (L_{\infty}-\bm{L_{r}})\left(1-e^{-K(t-\bm{t_{r}})}\right) }\]
  ...
  \begin{itemize}
    ...
    \pause
    \medskip
  \item \textbf{Importantly, $\bm{L_{r}}$ can be set to a critical length to estimate $\bm{t_{r}}$}.
\end{itemize}
\end{frame}
