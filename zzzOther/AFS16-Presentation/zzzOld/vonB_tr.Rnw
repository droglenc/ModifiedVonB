...
% sets the beamer theme and color
...
\end{itemize}
\bigskip
<<echo=FALSE, fig.width=3.5, fig.height=3.5, out.width='.4\\linewidth'>>=
par(mar=c(3.5,3.5,0.5,0.5),mgp=c(2.1,0.4,0),tcl=-0.2,las=1,xaxs="i",yaxs="i")
ages <- seq(-2,15,0.1)
...
\[\Scale[1.5]{ L_{t} = L_{0} + (L_{\infty}-L_{0})\left(1-e^{-K(t-\mathcolor{red}{0})}\right) }\]
\vspace{14pt}
<<redefineC, echo=FALSE, fig.width=4.5, fig.height=3.5, out.width='.5\\linewidth'>>=
<<redefineA>>
# Mark L0 on plot
...
df1 <- filterD(df,Stock=="WFM-01")
df2 <- filterD(df,Stock %in% c("WFM-01","WI-2"))
@
<<echo=FALSE, results="hide", warning=FALSE, message=FALSE>>=
## Fit the Models
Lr <- 0
...
par(mar=c(3.5,3.5,0.5,0.5),mgp=c(2.1,0.4,0),tcl=-0.2,las=1,xaxs="i",yaxs="i")
clrs <- c("black","red")
plot(TL~I(Age-0.05),data=filterD(df2,Stock=="WFM-01"),pch=16,col=col2rgbt(clrs[1],0.5),
     xlab="Age",ylab="Total Length (mm)",xlim=c(5,25),ylim=c(330,640),yaxt="n")
points(TL~I(Age+0.05),data=filterD(df2,Stock=="WI-2"),pch=16,col=col2rgbt(clrs[2],0.5))
...
legend("topleft",levels(df2$Stock),pch=16,lwd=2,col=clrs,bty="n",cex=0.8)
