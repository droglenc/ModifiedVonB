...
wae12 <- droplevels(subset(df,Year==2012))
...
svT <- list(Linf=650,K=0.2,t0=-2)
fitT <- nls(TL~vbT(Age,Linf,K,t0),data=wae12,start=svT)
bootT <- nlsBoot(fitT)
svO <- list(Linf=650,K=0.2,L0=200)
fitO <- nls(TL~vbO(Age,Linf,K,L0),data=bbn,start=svO)
bootO <- nlsBoot(fitO)
svN <- list(Linf=650,K=0.2,tr=3)
fitN <- nls(TL~vbnew(Age,Linf,K,tr,Lr),data=bbn,start=svN)
bootN <- nlsBoot(fitN)
